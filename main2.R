                    ...
                    dashboardHeader(title = "PBOD v 4.91",
                                    dropdownMenu(type = "messages",
                                                 messageItem(
                                                   ...
                                                   time = "2021-06-24"),
                                                 messageItem(
                                                   from = "Version 4.9 Update :",
                                                   message = "Remove Javascript bug update",
                                                   icon = icon("life-ring"),
                                                   time = "2021-05-23")
                                    )
                    ), 
                      ...
                      tabItems(
                        ...
                        tabItem(tabName = "currentbod",
                                fluidRow(
                                  ...
                                  #collapseInput(inputId = "isCollapseTransition",boxId = "ctransitionBox"),
                                  ...
                                  #collapseInput(inputId = "isCollapseDuration", boxId = "cdurationBox"),
                                  ...
                                  #collapseInput(inputId = "isCollapsecAction", boxId = "cActionBox"),
                        ...
                        tabItem(tabName = "newpbod",
                                ...
                                (
                                  ...
                                  #collapseInput(inputId = "isCollapsepTransition",boxId = "ptransitionBox"),
                                  box(id = "pdurationBox", title = "Duration parameter:",
                                      status = "info", solidHeader = TRUE,
                                      collapsible = TRUE,
                                      rHandsontableOutput("hot_pduration"),
                                      h6("NOTE: Leave blank or set zero in non-calculate cells.")),
                                  #collapseInput(inputId = "isCollapsepDuration",  boxId = "pdurationBox"),
...
server <- function(session,input, output) {
    ...
    if(Intervention == "Probabilistic"){
      ...
      if(Level=="ShowSubLevel"){
        ...
        if(TRUE){
          ...
          #   TransSheet[[cName]]<-TransSheet[[cName]]*PerNum
          # }
          #Check logic to Write file
          print("PM")
          showModal(modalDialog(
            tagList(
              textInput("txtPw", label = "Pass phrase", placeholder = "Key in Password to save file")
            ), 
            title="คุณต้องการที่จะบันทึกข้อมูลพารามิเตอร์ดังกล่าว หรือไม่? ",
            footer = tagList(actionButton("confirmSave", "Saving"),
                             modalButton("Cancel")
            )
          ))
          
          tfl<-tempfile("tmp/", fileext = c(".xlsx"))
          tmpFlname<<-substring(tfl,nchar(tempdir())+2)
          OriginalFlname<<-FlName
          
          write.xlsx(TransSheet, file=tmpFlname, sheetName="Trans", row.names=FALSE)
          write.xlsx(YLDSheet, file=tmpFlname, sheetName="YLD", append=TRUE, row.names=FALSE)
          write.xlsx(DWSheet, file=tmpFlname, sheetName="DW", append=TRUE, row.names=FALSE)
          write.xlsx(DurationSheet, file=tmpFlname, sheetName="Duration", append=TRUE, row.names=FALSE)
          write.xlsx(ShortWestTableMale, file=tmpFlname, 
                     sheetName="ShortWestTableMale", append=TRUE, row.names=FALSE)
          write.xlsx(ShortWestTableFemale, file=tmpFlname, 
  ...
  observeEvent(input$confirmSave, {
    tName<-req(input$txtPw)
    if(tName==OriginalPassCode){
    ...
    #   
      ...
      # print(tmpFlname)
      # print(OriginalFlname)
      file.copy(tmpFlname,OriginalFlname,overwrite = TRUE)
      file.remove(tmpFlname)
        shinyjs::alert("Saving file done!")
    }else{
       shinyjs::alert("Wrong passcode!!!")
      file.remove(tmpFlname)
    }
  ...
  shinyjs::disable('txtSpecialModuleName')
  ...
  observeEvent(input$chkSpecialModule,{
    tLogic<-input$chkSpecialModule
    if(tLogic){
      shinyjs::enable('txtSpecialModuleName')
    }else{
      shinyjs::disable('txtSpecialModuleName')
    }
  })
